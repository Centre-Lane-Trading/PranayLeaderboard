...
def toggle_area(relayoutData,n_clicks):
    ...
    if button_id =='reset-chart-toggle-button':
        ...
        if 'xaxis.range[0]' in relayoutData or 'yaxis.range[0]' in relayoutData:
                ...
                board.window_start = start_date_date_only
                board.window_end = end_date_date_only
                board.zoom_in(board.window_start,board.window_end)
                return dash.no_update, dash.no_update, {'display': 'block'}, dash.no_update
...
def handle_buttons(exclude_clicks, reset_clicks, start_date, end_date):
    ...
    exclusions = []  # List to store exclusion date ranges

    if button_id == "exclude-button":
        if start_date and end_date:
            # Create figure based on the original data
            fig = create_figure(
                board.original,  # Use the original data (no modification)
                x_col="date",
                y_col=board.chart_type,
        ...
        fig = create_figure(
            board.original,  # Use original data (no exclusions)
            x_col="date",
            y_col=board.chart_type,
            title="Leaderboard Data",
            x_label="Date",
